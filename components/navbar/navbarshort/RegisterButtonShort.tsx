...
const RegisterButtonShort = () => {

  const [buttonClicked, ClickButton] = useState(false);

  return (
    <Link href={ROUTES.REGISTER} onClick = {() => {ClickButton(!buttonClicked)}} className={`w-fit text-white text-[0.9rem] font-bold flex items-center justify-center rounded-[2px] bg-[linear-gradient(-45deg,#ff6ec4,#7873f5,#4ade80)]  bg-[length:200%_200%] animate-[gradientRotate_3s_ease_infinite] transition-transform duration-300 lg:hidden`}>
